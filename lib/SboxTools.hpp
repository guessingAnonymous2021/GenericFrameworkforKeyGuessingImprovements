#ifndef CONDITIONS_SRC_SBOXTOOLS_HPP_
#define CONDITIONS_SRC_SBOXTOOLS_HPP_
...
std::vector<std::vector<uint64_t>> ReadFile(std::ifstream & stream,
                                            size_t input_size,
                                            size_t output_size);

std::pair<std::vector<uint64_t>,
          std::vector<uint64_t>> LinearStructures(const VectorialBooleanFunction &cfun, size_t input_size);
...
CSVLine Analyse(const VectorialBooleanFunction& fun, bool print = false) {
  ...
  try {
    ...
    if(test_gs->leaves() != gs->leaves()) {
      ...
      std::cout << "avgcost: " << std::dec<< gs->AveragePathLength() << std::endl;
      std::cout << "leaves: " <<std::dec << gs->leaves() << std::endl;
      std::cout << *test_gs << std::endl;
      std::cout << "Test avgcost: " << std::dec << test_gs->AveragePathLength() << std::endl;
      std::cout << "Test leaves: " << std::dec << test_gs->leaves() << std::endl;
    }*/
...
std::vector<uint64_t> ReadFunctionNodelim(std::istream &stream,
                                          size_t input_size,
                                          size_t output_size);
std::vector<VectorialBooleanFunction> ReadFileNodelim(std::ifstream &stream,
                                                      size_t input_size,
                                                      size_t output_size,
                                                      ssize_t frst_line = 0,
                                                      ssize_t last_line = -1);
...
#endif //CONDITIONS_SRC_SBOXTOOLS_HPP_
