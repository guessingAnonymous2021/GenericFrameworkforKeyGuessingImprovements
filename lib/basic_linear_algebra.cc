...
NTL::mat_GF2 basic_linear_algebra::OrthogonalComplement(const NTL::mat_GF2 &matrix) {
...
void ListOfVectorSpacesRec(std::vector<std::vector<NTL::mat_GF2>> &spaces_bins,
  ...
  if (rank == row) {
    ...
    auto it = std::find_if(spaces_bins[row].begin(), spaces_bins[row].end(),
                           [copy](NTL::mat_GF2 &other) {
                             return other == copy;
                           });
...
bool basic_linear_algebra::RreLt(const NTL::mat_GF2 &mat,
  ...
  auto a = mat.NumRows();
  if (mat.NumRows() > other.NumRows()) a = other.NumRows();
  for (long i = 0; i < a; ++i) {
