#include "ExhaustiveBF.hpp"

ExhaustiveBF::BinaryDecisionTreeVector ExhaustiveBF::TreeSearch(const VectorialBooleanFunction &fun,
  ...
  if (BruteForce::IsConstantOnSubspace(fun,
                                       ...
                                       coset)) {
    result.push_back(std::move(std::make_unique<BinaryDecisionTree>(fun(coset))));
    return result;
  }
    ...
    for (const auto &left_tree: left_trees) {
      ...
      for (const auto &right_tree:right_trees) {
        ...
        BruteForce::BinaryDecisionTreePointer left_tree_copy =
            std::make_unique<BruteForce::BinaryDecisionTree>(left_tree->DeepImperfectCopy());
        BruteForce::BinaryDecisionTreePointer right_tree_copy =
            std::make_unique<BruteForce::BinaryDecisionTree>(right_tree->DeepImperfectCopy());
        auto candidate =
            std::make_unique<BinaryDecisionTree>(splitting_alpha,
                                                 std::move(left_tree_copy),
...
ExhaustiveBF::BinaryDecisionTreeVector ExhaustiveBF::StartSearch(const VectorialBooleanFunction &fun, bool print) {
  double bound = fun.InputSize() - 0.000001;
  return StartSearch(fun, bound, print);
}
ExhaustiveBF::BinaryDecisionTreeVector ExhaustiveBF::StartSearch(const VectorialBooleanFunction &fun,
