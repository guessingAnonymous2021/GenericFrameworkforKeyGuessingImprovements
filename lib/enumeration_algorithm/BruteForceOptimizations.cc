#include <random>
#include <set>
#include "BruteForceOptimizations.hpp"

std::set<uint64_t> BruteForceOptimization::zero_linear_structures(const VectorialBooleanFunction &fun,
                                             const Uint64Subspace& space, uint64_t offset) {
  std::set<uint64_t> ls;
  for (uint64_t xk = 0; xk < 1u << space.Dimension(); ++xk) {
    auto x = space.ElementK(xk);
    bool is_linear_structure = true;
    for (uint64_t yk = 0; yk < 1u << space.Dimension() && is_linear_structure; ++yk) {
      auto y = space.ElementK(yk)^offset;
      if ((fun(x^y)^ fun(y)) != 0) is_linear_structure = false;
    }
    if (is_linear_structure) ls.emplace(x);
  }
  return ls;
}

BruteForceOptimization::BinaryDecisionTreePointer BruteForceOptimization::TreeSearch(const VectorialBooleanFunction &fun,
  ...
  if (BruteForce::IsConstantOnSubspace(fun,
                           linear_domain,
                           coset)) {
  ...
  for (uint64_t k = 1; k < num; ++k) {
    auto splitting_alpha =
        basic_linear_algebra::ConvertToUint(basic_linear_algebra::Embed(k,
                                                                        possible_masks));
    ...
    // (A representation of) the difference between cosets of the next level is q.
    // If two sister nodes only differ by a linear structure, there is a better tree
    // (with fewer levels). q might be a bad choice of representation though
    ...
    if (differs_by_linear_structure) continue;
    ...
    // Build the subtrees:
    auto new_bound = std::min(2 * bound - 2,
                              double(fun.InputSize()
                                         - new_vectors_on_path.NumRows()));
    auto left_tree =
    ...
    new_bound = std::min(2 * bound - 2 - left_tree->associated_cost_,
                         double(fun.InputSize()
                                    - new_vectors_on_path.NumRows()));
    auto right_tree =
        TreeSearch(fun,
    ...
    if (total_costs > bound) continue;
    if (!result) {
      result =
          std::make_unique<BruteForce::BinaryDecisionTree>(splitting_alpha,
                                               std::move(left_tree),
                                               std::move(right_tree));
      ...
      if (level == 0 && print) {
        std::cout << "Best tree so far\n"
                  << *result << "\nwith costs: " << result->associated_cost_ << std::endl;
      }
    } else {
      auto candidate =
          std::make_unique<BinaryDecisionTree>(splitting_alpha,
                                               std::move(left_tree),
                                               std::move(right_tree));
      if (candidate
          && (total_costs < result->associated_cost_)) {
...
BruteForceOptimization::BinaryDecisionTreePointer BruteForceOptimization::StartSearchWithFixedRoot(const VectorialBooleanFunction &fun,
                                                              double &bound,
                                                              uint64_t root, bool print) {
  ...
  if (i == 0) {
    throw std::logic_error("Bad luck, broken random device or insanely high dimension.");
  }
  vectors_on_path[0] = basic_linear_algebra::ConvertToNtl(root, vectors_on_path.NumCols());
  double left_bound = std::min(2* bound - 2, double(fun.InputSize() - 1));
  auto left_tree = TreeSearch(fun, vectors_on_path, choices, 0, coset, left_bound, 1, print);
  if (!left_tree) {
    throw std::logic_error("Nothing found.");
  }
  double right_bound = std::min(2*bound - left_tree->associated_cost_ - 2, double(fun.InputSize() - 1));
  choices[0] = 1;
  coset = basic_linear_algebra::ConvertToNtl(candidate, fun.InputSize());
  auto right_tree = TreeSearch(fun, vectors_on_path, choices, 0, coset, right_bound, 1, print);
  if (!right_tree) {
    throw std::logic_error("Nothing found.");
  }
  auto total_cost = 1+0.5*(left_tree->associated_cost_ + right_tree->associated_cost_);
  auto result = std::make_unique<BinaryDecisionTree>(root,
                                                     std::move(left_tree),
                                                     std::move(right_tree));
...
BruteForceOptimization::BinaryDecisionTreePointer BruteForceOptimization::StartSearch(const VectorialBooleanFunction &fun,
  ...
  NTL::vec_GF2 choices;
  choices.SetLength(fun.InputSize()); // I hope zero initializes
  return TreeSearch(fun, vectors_on_path, choices, -1, choices, bound, 0, print);
}

BruteForceOptimization::BinaryDecisionTreePointer BruteForceOptimization::StartSearch(const VectorialBooleanFunction &fun, bool print) {
  double bound = fun.InputSize();
  return StartSearch(fun, bound, print);
}

void
BruteForceOptimization::RecursiveStump
(
  ...
  auto total_cost = 1+0.5*(stump->left_->associated_cost_ + stump->right_->associated_cost_);
  stump->associated_cost_ = total_cost;
}

BruteForceOptimization::BinaryDecisionTreePointer BruteForceOptimization::StartSearchWithFixedStump(
    const VectorialBooleanFunction &fun,
    const BruteForceOptimization::BinaryDecisionTreePointer &stump,
    double &bound,
    uint64_t root,
  ...
  BruteForceOptimization::BinaryDecisionTreePointer temp =
      std::make_unique<BruteForceOptimization::BinaryDecisionTree>(stump->DeepImperfectCopy()) ;
  // Not efficient, but I don't care
  // Just go through the leaves in-order and start the optimal search with that bound everywhere.
  NTL::mat_GF2 vectors_on_path;
  vectors_on_path.SetDims(0, fun.InputSize());
  assert(vectors_on_path.NumCols() == fun.InputSize());
  NTL::vec_GF2 choices;
  choices.SetLength(fun.InputSize()); // I hope zero initializes
  RecursiveStump(fun, temp, vectors_on_path, choices, -1, choices, bound, 0, print);
  return temp;
}
BruteForceOptimization::BinaryDecisionTreePointer BruteForceOptimization::generate_stump(
    std::vector<uint64_t>::iterator fixed_bits_begin, std::vector<uint64_t>::iterator fixed_bits_end) {
  if (fixed_bits_begin == fixed_bits_end) return nullptr;
  BruteForceOptimization::BinaryDecisionTreePointer left = BruteForceOptimization::generate_stump(fixed_bits_begin + 1, fixed_bits_end);
  BruteForceOptimization::BinaryDecisionTreePointer right;
  if (left)
     right = std::make_unique<BruteForceOptimization::BinaryDecisionTree>(left->DeepImperfectCopy());
  return std::make_unique<BruteForceOptimization::BinaryDecisionTree>(*fixed_bits_begin,
                                              std::move(left),
                                              std::move(right));
}
BruteForceOptimization::BinaryDecisionTreePointer BruteForceOptimization::fix_bits(
    std::vector<uint64_t> fixed_bits) {
