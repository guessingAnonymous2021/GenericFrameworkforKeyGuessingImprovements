...
#include "MinLeaves.hpp"

MinLeaves::BinaryDecisionTreePointer MinLeaves::TreeSearch(const VectorialBooleanFunction &fun,
  ...
  auto ls0 = BruteForceOptimization::zero_linear_structures(fun, linear_domain, coset);
  ...
  for (uint64_t k = 1; k < num; ++k) {
    ...
    // Build the subtrees:
    auto new_bound = std::min(bound, std::pow(2.0, fun.InputSize() - new_vectors_on_path.NumRows()));
    ...
    new_bound = std::min(bound - left_tree->associated_cost_, std::pow(2.0, fun.InputSize() - new_vectors_on_path.NumRows()));
    auto right_tree =
        TreeSearch(fun,
                   new_vectors_on_path,
                   new_choices,
    ...
    const auto total_costs = left_tree->associated_cost_ +
        right_tree->associated_cost_;
    if (total_costs > bound) continue;
    if (!result) {
      result =
          std::make_unique<MinLeaves::BinaryDecisionTree>(splitting_alpha,
                                               std::move(left_tree),
                                               std::move(right_tree));
...
MinLeaves::BinaryDecisionTreePointer MinLeaves::StartSearchWithFixedRoot(const VectorialBooleanFunction &fun,
                                                              double &bound,
  ...
  double right_bound = std::min(bound - left_tree->associated_cost_, std::pow(2.0, fun.InputSize() - 1));
  choices[0] = 1;
...
MinLeaves::BinaryDecisionTreePointer MinLeaves::StartSearch(const VectorialBooleanFunction &fun,
...
MinLeaves::BinaryDecisionTreePointer MinLeaves::StartSearch(const VectorialBooleanFunction &fun, bool print) {
  double bound = std::pow(2.0, fun.InputSize());
  return StartSearch(fun, bound, print);
}

void
MinLeaves::RecursiveStump
(
  ...
  // Build the subtrees:
  auto new_bound = std::min(bound,
                            std::pow(2.0, fun.InputSize()
  ...
  new_choices[last_choice + 1] = 1;
  new_bound = std::min(bound - stump->left_->associated_cost_,
                       std::pow(2.0, fun.InputSize()
                                  - new_vectors_on_path.NumRows()));
  if (stump->right_) {
    RecursiveStump(
  ...
  auto total_cost = stump->left_->associated_cost_ + stump->right_->associated_cost_;
...
MinLeaves::BinaryDecisionTreePointer MinLeaves::StartSearchWithFixedStump(
  ...
  MinLeaves::BinaryDecisionTreePointer temp =
      std::make_unique<MinLeaves::BinaryDecisionTree>(stump->DeepImperfectCopy()) ;
