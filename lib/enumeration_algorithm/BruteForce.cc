...
BruteForce::BinaryDecisionTreePointer BruteForce::TreeSearch(const VectorialBooleanFunction &fun,
  ...
  if (IsConstantOnSubspace(fun,
                           Uint64Subspace(space, space.NumCols()),
                           coset)) {
...
BruteForce::BinaryDecisionTreePointer BruteForce::StartSearchWithFixedRoot(const VectorialBooleanFunction &fun,
                                                              double &bound,
...
BruteForce::BinaryDecisionTreePointer BruteForce::StartSearch(const VectorialBooleanFunction &fun,
...
BruteForce::BinaryDecisionTreePointer BruteForce::StartSearch(const VectorialBooleanFunction &fun, bool print) {
...
void BruteForce::analyse_component(const VectorialBooleanFunction &function,
  ...
  try {
    ...
    std::cout << *gs << '\n';
    std::cout << "Reconstructed function " << gs->UnderlyingFunction(function.InputSize())
              << '\n';
...
void BruteForce::BinaryDecisionTree::recursive_print(std::ostream &os, int i) {
  print_spaces(os, i);
  if (IsLeaf()) os << "Constant = ";
  os << std::hex << value_ << "\n";
  if (left_) left_->recursive_print(os, i + 1);
  if (right_) right_->recursive_print(os, i + 1);
}
...
size_t BruteForce::BinaryDecisionTree::BitSize() const {
  if (IsLeaf()) return 0;
  return std::max<size_t>({64u - std::countl_zero(value_),
                           left_->BitSize(),
                           right_->BitSize()});
}
size_t BruteForce::BinaryDecisionTree::OutputBitSize() const {
  if (IsLeaf()) return 64u - std::countl_zero(value_);
  return std::max<size_t>(left_->OutputBitSize(),
                          right_->OutputBitSize());
}
VectorialBooleanFunction BruteForce::BinaryDecisionTree::UnderlyingFunction(
  ...
  size_t output_bit_size = output_size_hint ? output_size_hint : OutputBitSize();
  std::vector<uint64_t> base(1u << bit_size);
  std::iota(base.begin(), base.end(), 0);
  std::transform(base.begin(), base.end(), base.begin(), [&](auto x) {
    return EvaluateAt(x);
  });
