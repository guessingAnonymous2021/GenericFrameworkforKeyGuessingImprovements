...
std::vector<uint64_t> SboxTools::ReadFunctionNodelim(std::istream &stream,
  ...
  std::vector<uint64_t> values (1u << input_size );
  uint64_t i = 0;
  while (i < 1u << input_size && stream.get(current_value) && i < (1u << input_size)) {
    if (!isdigit(current_value)) break; // ignore non-decimal digits
    // sufficient here
...
std::vector<VectorialBooleanFunction> SboxTools::ReadFileNodelim(std::ifstream &stream,
                                                                 size_t input_size,
                                                                 size_t output_size,
                                                                 ssize_t frst_line,
                                                                 ssize_t last_line) {
  ...
  while(std::getline(stream, line)) {
    line_number++;
    if (line_number < frst_line) continue;
    if (last_line >= 0 && line_number >= last_line) break;
    std::istringstream iss(line);
    result.emplace_back(ReadFunctionNodelim(iss, input_size, output_size), input_size, output_size);
  }
  return result;
}
...
std::pair<std::vector<uint64_t>,
          std::vector<uint64_t>> SboxTools::LinearStructures(const VectorialBooleanFunction &cfun,
                                                             size_t input_size) {
...
std::ostream &SboxTools::operator<<(std::ostream &os, const SboxTools::csv_line &line) {
  os << line.function << ",";
