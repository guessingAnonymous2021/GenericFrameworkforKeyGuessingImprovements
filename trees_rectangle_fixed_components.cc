...
#include "lib/ComponentChoice.hpp"
...
std::vector<uint64_t> rectangle_new_sbox{
...
void analyse_pairs(std::ostream& out, VectorialBooleanFunction & v, std::vector<uint64_t> outputs) {
  ...
  for (size_t i = 1; i < (1u << v.InputSize()); ++i) {
    for (size_t j = 1+i; j < (1u << v.InputSize()); ++j) {
      ...
      auto gs = BruteForceOptimization::StartSearchWithFixedStump(cfun, stump, bound, 0, false);
      out << *gs << "\n------------------" << std::endl;
    }
  }
}
